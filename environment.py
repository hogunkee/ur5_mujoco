...
class lab_env():
    def __init__(self, args):
        ...
        self.model = load_model_from_path('UR5_w_robotiq.xml')
        self.sim = MjSim(self.model)
        self.viewer = MjViewer(self.sim)
        self.viewer._hide_overlay = True

    def step(self, action, grasp=0.0):
        ...
        self.sim.data.qpos[3] = remap(action[3], -1, 1, -30 / 180 * math.pi, 45 / 180 * math.pi)
        self.sim.data.qpos[4] = remap(action[4], -1, 1, -30 / 180 * math.pi, 45 / 180 * math.pi)
        self.sim.data.qpos[5] = remap(action[5], -1, 1, -30 / 180 * math.pi, 45 / 180 * math.pi)"""
        #self.sim.data.ctrl[0:6] = action[0:6]
        self.sim.data.qpos[0:6] = action[0:6]
        #self.sim.data.qpos[6:9] = grasp
        #self.sim.data.qpos[7] = grasp
        #self.sim.data.qpos[11] = grasp
        #self.sim.data.qpos[15] = grasp
        #self.sim.data.qpos[7:10] = grasp
        #self.sim.data.qpos[11:15] = grasp
        #self.sim.data.qpos[15] = grasp
        #self.sim.data.qpos[5] = remap(action[5], -1, 1, -30 / 180 * math.pi, 45 / 180 * math.pi)
        self.sim.data.ctrl[6:10] = grasp
    ...
    def get_state(self):
        ...
        danger = int(self.safety_check())
        #return (int(self.grasping == self.task), danger)
        return danger
